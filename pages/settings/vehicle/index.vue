    ...
    <UModal v-model="modalAdd" :ui="{ width: 'sm:max-w-6xl'}" @close="closeModal">
        <UForm :state="form" @submit="submit">
            ...
            >
                ...
                <UFormGroup label="ประเภทรถยนต์" class="flex space-x-2 mb-4" size="xl" :ui="uiFormGroup">
                ...
                <UFormGroup label="คำอธิบาย" class="flex space-x-2 mb-4" size="xl" :ui="uiFormGroup">
                    <UInput placeholder="" v-model="form.type_desc" required />
                </UFormGroup>
                <template #footer>
...
<script setup>  
    ...
    const dataForm = {
        type_key:"VEHICLE",//VEHICLE , REASON , VEHICLE
        type_id: genID(),
    ...
    const edit = async (id) => {
        const data = await getApi(`/bk/type/GetDocSet?type_key=VEHICLE&type_id=${id}`)
